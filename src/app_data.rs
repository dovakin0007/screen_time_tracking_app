...
impl AppData {
    ...
    pub fn store_data(
        ...
        Self {
            ...
            hours_spent,
            minutes_spent,
            current_day: today_date.to_string(),
        }
    }
...
pub async fn update_db(data: &HashMap<String, AppData>) -> Result<(), std::io::Error> {
    let database_url =
        Opts::from_url("mysql://root:dOVAKIN03!@localhost/screen_time_monitoring").unwrap();
    ...
    r"REPLACE INTO monitoring_table VALUES (
        ...
        :minutes_spent,
        :current_day);"
        .with(data_vec.iter().map(|curr_data| {
            params! {
...
pub async fn get_data_from_db<'a>(
    data_map: &'a mut HashMap<String, AppData>,
    today_date: &NaiveDate,
) -> Result<&'a mut HashMap<String, AppData>, std::io::Error> {
    let database_url =
        Opts::from_url("mysql://root:dOVAKIN03!@localhost/screen_time_monitoring").unwrap();
    ...
    from monitoring_table where current_day=:today"
        .with(params! {
            "today" => curr_day
        })
        .map(
            &mut conn,
            |(app_id, app_name, seconds_spent, hours_spent, minutes_spent)| {
    ...
    let _ = loaded_data
        .iter()
        .map(|app_data| data_map.insert(app_data.app_name.clone(), app_data.to_owned()))
        .collect::<Vec<_>>();
